...
private function handleProductPurchase($session)
{
    ...
    try {
        ...
        // Send welcome email with discount code for gold/platinum packages
        try {
            Mail::to($client->email)->send(new WelcomePackageMail($client, $packageName));
            Log::info('Welcome email sent', [
                'email' => $client->email,
                'package' => $packageName,
                'has_discount' => in_array($packageName, ['gold', 'platinum'])
            ]);
        } catch (\Exception $e) {
