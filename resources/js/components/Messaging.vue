    ...
    <div class="send-message-form">
      ...
      <form @submit.prevent="sendMessage">
        ...
        <button
          type="submit"
          :disabled="sending"
          class="w-full py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed transition duration-200"
        >
...
export default {
  ...
  methods: {
    /**
     * Fetch messages for the authenticated user, excluding any involving the admin.
     */
    async fetchMessages() {
      this.loading = true;
      try {
        ...
        if (response.data.success) {
          // Filter out messages involving admin (ID: 1)
          this.messages = response.data.data.filter(msg => 
            msg.sender_id !== this.adminId && msg.receiver_id !== this.adminId
          );
          console.log('Filtered Messages:', this.messages); // Debugging
        } else {
    ...
    async fetchUsers() {
      try {
        ...
        if (this.user.role === 'matchmaker') {
          ...
          if (response.data.success) {
            // Exclude admin from the users list if admin is considered a client
            this.users = response.data.data.filter(user => user.id !== this.adminId);
            console.log('Fetched Clients:', this.users); // Debugging
          } else {
          ...
          if (response.data.success) {
            // Exclude admin from the matchmakers list if necessary
            this.users = response.data.data.filter(user => user.id !== this.adminId);
            console.log('Fetched Matchmakers:', this.users); // Debugging
          } else {
    ...
    getOtherUser(message) {
      if (message.sender_id === this.user.id) {
        return this.users.find(user => user.id === message.receiver_id) || { name: 'Unknown' };
      } else {
        return this.users.find(user => user.id === message.sender_id) || { name: 'Unknown' };
      }
    },
