<!-- resources/js/components/CandidateApproval.vue -->
<template>
  <div class="p-8 bg-gray-100 min-h-screen">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="candidate in candidates" :key="candidate.id" class="p-6 bg-white shadow-md rounded-lg">
        <h2 class="text-lg font-semibold text-[#213366]">{{ candidate.name }}</h2>
...
export default {
  ...
  methods: {
    async fetchCandidates() {
        ...
        await axios.get('/api/admin/candidates')
          .then((response) => {                                       
    ...
    async approveCandidate(userId) {
      this.processing = true
        axios.defaults.headers.common.Authorization = `Bearer ${this.authorization.token}`        
        await axios.post('/api/admin/candidates/approve', {                
          user_id: userId,
        })
        .then(({data})=>{                
