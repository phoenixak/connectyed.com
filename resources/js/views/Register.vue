  ...
  <div class="max-w-lg mx-auto p-4 bg-white relative">
    ...
    <div class="flex items-center mb-6">
      <div v-for="(step, index) in steps" :key="index">
        <div
          @click="goToStep(step)"
          class="text-center py-2 px-3 mx-1 rounded-full cursor-pointer transition-colors duration-200"
          :class="{
            'bg-blue-500 text-white': currentStep === step,
            'bg-gray-200 text-gray-600 hover:bg-gray-300': currentStep !== step
          }"
        >
          {{ step }}
        </div>
    ...
    <form @submit.prevent="register" enctype="multipart/form-data">
      ...
      <div class="mt-6 flex justify-between">
        ...
        <button
          v-else
          :class="[
            'py-2 px-4 rounded min-w-32 transition-colors duration-200',
            processing 
              ? 'bg-gray-400 cursor-not-allowed'
              : 'bg-connectyed-button-light hover:bg-connectyed-button-hover-light text-connectyed-button-hover-dark cursor-pointer'
          ]"
          type="submit"
          :disabled="processing"
        >
...
export default {
  ...
  methods: {
    ...
    async register() {
      if (this.processing) return; // Prevent multiple submissions
...
/* Add these styles for step indicators */
.bg-blue-500 {
  background-color: var(--connectyed-button-light, #4A90E2);
}
...
.bg-connectyed-button-light {
  background-color: #4A90E2; /* Adjust this color to match your theme */
}

.bg-connectyed-button-dark {
  background-color: #357ABD; /* Added for switch background when active */
}
...
.text-connectyed-button-dark {
  color: #4A90E2; /* Text color when button is not hovered */
}
...
.bg-gray-400 {
  background-color: #A0AEC0; /* Adjust as needed */
}
